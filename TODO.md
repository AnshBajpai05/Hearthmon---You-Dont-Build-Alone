...
> Start of the `docs/dev/15_jun.md` **V1 Polish Day → Art Direction** pass, on the Alive renderer's cosmos/observatory corner. (Day-15 itself went to the security/chapter gate, so the polish plan slipped to here.)
...
- [ ] **Remaining Art Direction pass:** layer foreground/mid/background + atmosphere on every biome (electric ground done, see below); push **Moonlit Shore** to screenshot-worthy. Then the two CRITICAL `docs/dev/15_jun.md` passes still untouched: **Writing/Dialogue** (`lib/lines.ts`) and **Interaction smoothness**.
